...
export default class SyncTool {
  ...
  cursorPositionChanged (index, e) {
    ...
    let otherBufferPosition = newBufferPosition
    if (thisWrap.editor.isSoftWrapped()) {
      const firstBufferPosition = [newBufferPosition.row, 0]
      const firstScreenPosition = thisWrap.editor.screenPositionForBufferPosition(firstBufferPosition)
      let rowOffset = newScreenPosition.row - firstScreenPosition.row
      const otherBufferRange = otherWrap.editor.getBuffer().rangeForRow(newBufferPosition.row)
      const otherScreenRange = otherWrap.editor.screenRangeForBufferRange(otherBufferRange)
      const otherRowCount = otherScreenRange.getRowCount()
      rowOffset = Math.min(rowOffset, otherRowCount - 1)
      const otherScreenPosition = otherScreenRange.start
      otherScreenPosition.row += rowOffset
      otherBufferPosition = otherWrap.editor.bufferPositionForScreenPosition(otherScreenPosition)
    }
    otherWrap.editor.setCursorBufferPosition(otherBufferPosition)
  ...
  highlightCurrentLine () {
    this.markers = this.editorWraps.map((editorWrap, i) => {
      ...
      if (prevMarker) {
        const prevRange = prevMarker.getScreenRange()
